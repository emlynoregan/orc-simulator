...
def get_orc_message2(is_hungry, human_has_chicken):
    hungry_message = (f"You are hungry, and feeling aggressive." if human_has_chicken else "You are hungry, and the human ate the chicken and didn't give you any! You should kill him with the sword!") if is_hungry else "You are not hungry. The nice human gave you chicken."  

    orc_prefix = f"""You are playing an Orc in a roleplaying game. The orc is very superstitious and afraid of curses, and intrigued by the human.
You are in a dark, wet cave.
A human has arrived.{" The human is holding a cooked chicken." if human_has_chicken else ""}
You have a sword, and are a strong fighter.{" You could smash the human easily." if is_hungry else ""} 
You have an amulet{", which you do not wish to lose." if is_hungry else ", which you might be willing to give away if it was a good idea."}
{hungry_message}
You are not very smart{", but you think the human might be trying to trick you." if is_hungry else ". Because you are now full of chicken, you feel happier, and will believe the human."}
When you speak, you speak like an Orc. You might say "me hungry" or "me smash you human!" or "me like you human" or "me no like stinky dark cave".
...
Please provide a comma separated list of three values: 
The first one is how much the Orc hates the human, from 1 to 10. 
The second one is whether the Orc will give the Human the amulet (Y/N). 
And the third one is the next line of dialog for the Orc.

Your response:"""

    prompt = orc_prefix + chat_history + orc_suffix
...
def get_orc_says():    
    ...
    orc_prompt = get_orc_message2(data["orc"]["hungry"], "chicken" in data["human"]["items"])
    ...
    while max_tries and not orc_answer:
        try:
        ...
        # Please provide a comma separated list of three values: 
        # The first one is whether the Orc will attack the Human with the sword (Y/N). 
        # The second one is whether the Orc will give the Human the amulet (Y/N). 
        # And the third one is the next line of dialog for the Orc."""

            # split into maximum 3 parts
            attack_answer, give_answer, orc_answer = ai_msg.split(",", 2)
    ...
    will_attack = attack_answer.strip().lower() in ["9", "10"]

    will_give = give_answer.strip().lower() in ["y", "yes"]

    # remove quotes from the orc answer if they're there
    orc_says = orc_answer.strip().strip('"')
    
    return will_attack, will_give, orc_says
...
while True:
    ...
    if not skip_orc:
        ...
        will_attack, will_give, orc_says = get_orc_says()
        ...
        if orc_has_sword:
            ...
            orc_action = "attacks" if will_attack else None
        ...
        if not orc_action:
            ...
            if orc_has_amulet:
                ...
                orc_action = "gives amulet" if will_give else None
